...
import { useDbIconOverrides } from "../../_lib/db-icon-overrides";
...
export function MobileHomeScreen() {
  ...
  const { user } = useAuth();
  const dbIcons = useDbIconOverrides();
  ...
  // Anon = all non-hidden apps (matches Desktop.tsx). Logged-in = mode filter.
  const icons = useMemo(() => {
              ...
              {pageIds.map((id, slotIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <img
                        src={dbIcons.get(app.id) ?? app.icon}
                        alt=""
                        draggable={false}
                        style={{
                          width: 44,
