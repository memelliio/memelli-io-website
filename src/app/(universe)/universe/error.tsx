...
/**
 * Universe-level error boundary.
 * Catches render errors in any /universe/* page and auto-reports them
 * to the Navigation Health Monitor API.
 */
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch(`${API_URL}/api/admin/nav-health/report`, {
        ...
        // Silently fail — reporting should not block the user
      });
    }
  }, [error]);
    ...
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 text-center">
      ...
      <div>
        ...
        <p className="mt-2 max-w-md text-sm text-[hsl(var(--muted-foreground))]">
          This error has been automatically reported to the Navigation Health Monitor.
        </p>
        {error.message && (
          <p className="mt-2 max-w-md rounded-lg bg-[hsl(var(--card))] px-4 py-2 font-mono text-xs text-red-400">
            {error.message}
          </p>
        )}
      </div>
