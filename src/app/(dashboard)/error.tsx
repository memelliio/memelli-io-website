 ...
 * Catches unhandled render errors in any /dashboard/* page
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 text-center p-8">
      ...
      <div>
        ...
        {error.message && (
          <p className="mt-2 max-w-md rounded-lg bg-zinc-900 px-4 py-2 font-mono text-xs text-red-400/80">
            {error.message}
          </p>
        )}
      </div>
      <button
        onClick={reset}
        className="rounded-xl bg-white/[0.06] border border-white/[0.08] px-6 py-2.5 text-sm font-medium text-zinc-300 hover:bg-white/[0.10] hover:text-white transition-all duration-200"
      >
        Try again
      </button>
    </div>
